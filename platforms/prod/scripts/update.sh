...
OLD[0]="prod-db"
PROD[0]="docker run -d --volumes-from dbdata --name ${OLD[0]} softinnov/${OLD[0]}"
OLD[1]="prod-esc-pdv"
...
OLD[5]="prod-client"
PROD[5]="docker run -d --link prod-esc-pdv:esc-pdv --link prod-esc-adm:esc-adm --link prod-esc-caisse:esc-caisse --link prod-back:back -v $(pwd)/logs:/var/log/nginx -p 8000:8000 --name ${OLD[5]} softinnov/${OLD[5]}"
