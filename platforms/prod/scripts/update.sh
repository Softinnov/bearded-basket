...
PROD[0]="docker run -d --volumes-from dbdata -v $(pwd)/data/:/data --name prod-db softinnov/prod-db"
OLD[0]="prod-db"
...
OLD[2]="prod-back"
PROD[3]="docker run -d --link prod-chey:chey --link prod-back:back -v $(pwd)/logs:/var/log/nginx -p 80:8000 -p 443:443 --name prod-client softinnov/prod-client"
OLD[3]="prod-client"
